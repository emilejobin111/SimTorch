...
if __name__ == "__main__":
    ...
    neural_network.init_radom_params(jax.random.key(12))
    ...
    x = jnp.linspace(-5, 5, 128,dtype=jnp.float32)
    y = jnp.linspace(-5, 5, 128,dtype=jnp.float32)
    ...
    @jax.jit
    def train(net:nn.JaxModule, optimiser:optim.AdamW, X, Y):
        train_func = lambda _, net, optim : epoch_step(net, optim, X, Y)[0:2]
        trained_net, _ = jax.lax.fori_loop(0,1000,train_func,(net,optimiser))
        return trained_net
    ...
    # train(neural_network,optimiser,X_train,Y_train)
    # t = time.perf_counter()
    # neural_network = train(neural_network,optimiser,X_train,Y_train)
    # neural_network = jax.block_until_ready(neural_network)
    # print(time.perf_counter()-t)

    # predicted = neural_network.forward(X_train)
    # plot_3d_surface(X_train, predicted)
    _ = epoch_step(neural_network, optimiser, X_train, Y_train)
    t = time.perf_counter()
    for i in range(1000):
        neural_network, optimiser, loss = epoch_step(neural_network, optimiser, X_train, Y_train)
        # if i % 100 == 0:
